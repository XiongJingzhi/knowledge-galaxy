...
- 文档页收敛为索引台，负责搜索、筛选弹层和带真实更新时间的表格列表
- 文档创建与编辑进入独立二级工作区，采用左侧 Markdown 编辑、右侧预览，时间字段自动生成
- 侧栏加入桌面端专用的星系 logo，延续深色结构网格与轨道视觉语言
- 文档页支持浏览、搜索、弹层筛选，以及进入二级编辑工作区；编辑页的滚动已收敛到主内容区，不再让整页壳层一起滚动
- 资源页支持 `asset-list` 浏览、按仓库级 / 项目级切换、系统文件选择导入、`md/zip` 知识迁移，以及查看资源详情元数据
...
知识迁移当前通过本地 `Ollama` 模型完成结构化分类，先生成可编辑的迁移预览；候选项可展开审阅原始来源与 Markdown 正文，再确认写入知识库标准目录，并直接回到文档编辑器继续精修。
