...
- `export asset-list`：
  导出当前仓库下的资源清单，覆盖 `assets/` 与 `projects/*/assets/`，并包含 `sha256`
...
- `stats` 输出总量、类型分布、状态分布、主题分布、标签频率、来源分布

规划中但尚未实现：

- 更丰富的聚合视图，例如联合分组和跨维度摘要
...
- `export` 输出 JSON，其中 `document-list` 返回路径排序结果，`manifest` 返回整体快照，`change-list` 返回按 `updated_at` 倒序排列的当前变更视图，`asset-list` 返回带 `sha256` 的当前资源清单

### 8.4 当前仍未实现

以下能力仍在 1.0 计划中，但截至 2026-03-13 尚未落地：

- 更完整的资源生命周期工作流，例如自动去重策略
